...
from data_reader import DataReader#_gat
from models import GNN#_gat
...
date = 'csv_sum_2cls_800_22cls_rearranged'#此处为document中的日期文件夹名称#csv_sum_22cls
...
parser.add_argument('--epochs', type=int, default=100, help='Number of epochs to train.')#原本是5
...
for fold_id in range(args.n_folds):#遍历不同fold，但此处只有1个fold，即fold 0，也就是说采用第一折作为测试集的做法
    ...
    model = GNN(loaders[0].dataset.features_dim,#独热
                ...
                output_dim=loaders[0].dataset.n_classes,#outdim为22
    ...
    def test(test_loader):
        ...
        with torch.no_grad():#不计算梯度
            ...
            for batch_idx, data in enumerate(test_loader):
                ...
                testtime1 = time.time()
                ''''''
                #input(f"一个batch耗时为：{testtime1 - testtime0}")
                #input(f"一个batch中样本数量为：{len(data[0])}")
                #input(f"一个batch中每个样本耗时为：{(testtime1 - testtime0) / len(data[0])}")
            ...
            #input(f"classnums长度为: {classnums}")
            '''
            ...
            #input(f"唯一的标签值：{unique_labels}")
            ######################################################################
            conf_matrix = get_confusion_matrix(labels.view_as(preds), preds)#生成矩阵
    ...
    def plot_confusion_matrix(conf_matrix, num_classes, epoch):
        ...
        if epoch == 0:
            plt.savefig('./No_inc/fig0.png', format='png')
        if epoch == 1:
            plt.savefig('./No_inc/fig1.png', format='png')
        if epoch == 2:
            plt.savefig('./No_inc/fig2.png', format='png')
        if epoch == 3:
            plt.savefig('./No_inc/fig3.png', format='png')
        if epoch == 4:
            plt.savefig('./No_inc/fig4.png', format='png')
        if epoch == 5:
            plt.savefig('./No_inc/fig5.png', format='png')
        if epoch == 6:
            plt.savefig('./No_inc/fig6.png', format='png')
        if epoch == 7:
            plt.savefig('./No_inc/fig7.png', format='png')
        if epoch == 8:
            plt.savefig('./No_inc/fig8.png', format='png')
        if epoch == 9:
            plt.savefig('./No_inc/fig9.png', format='png')
        plt.show()
        plt.close()#关闭图像释放内存
    ...
    print("仅训练时间: {:.4f}s".format(time.time() - t_start))
    acc_folds.append(max_acc)
    print(f"总时间: {time.time() - time0}")
print(acc_folds)
print(f"最大acc出现的epoch为{acc_max_epoch}")
