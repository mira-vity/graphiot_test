...
import math
import numpy as np
from encoder_lite import supconGNN#导入模型_ctrled
from losses_negative_only import SupConLoss#导入损失函数

def set_model(args, init_dim=2156):#模型设置，原属于util，由于从util38copy过来，所以原本是1461或2156，现修正为1472
    model = supconGNN(
                input_dim=init_dim,#init_dim需要调整输入维度
