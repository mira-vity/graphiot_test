...
date = 'csv_sum_adapted1_rearranged'#此处为document中的日期文件夹名称 #csv_sum_2cls_800_22cls_rearranged #csv_sum_22cls
...
parser.add_argument('--epochs', type=int, default=1, help='Number of epochs to train.')#初始为5
...
for fold_id in range(args.n_folds):
    ...
    def test(test_loader, old_test_loader=[], new_test_loader=[], stage=0):
        model.eval()
        start = time.time()
        with torch.no_grad():
    ...
    acc_folds0.append((max_acc, max_acc_old, max_acc_new))
    ...
    print('\nFOLD', fold_id)
    loaders1 = []
    for split in ['train1', 'test1', 'new1']:
    ...
    for epoch in range(args.epochs):
        train(loaders1[0])
        acc,acc_old,acc_new = test(loaders1[1],loaders0[0],loaders1[2],1)
    ...
    acc_folds1.append((max_acc, max_acc_old, max_acc_new))
    ...
    print('\nFOLD', fold_id)
    loaders2 = []
    for split in ['train2', 'test2', 'new2']:
    ...
    for epoch in range(args.epochs):
        train(loaders2[0])
        acc,acc_old,acc_new = test(loaders2[1],loaders1[0],loaders2[2],2)
    ...
    acc_folds2.append((max_acc, max_acc_old, max_acc_new))
    ...
    print('\nFOLD', fold_id)
    loaders3 = []
    for split in ['train3', 'test3', 'new3']:
    ...
    for epoch in range(args.epochs):
        train(loaders3[0])
        acc,acc_old,acc_new = test(loaders3[1],loaders2[0],loaders3[2],3)
    ...
    acc_folds3.append((max_acc, max_acc_old, max_acc_new))
