...
from graph_data_inc_22 import GraphData
from data_reader_inc_22 import DataReader, shared_params
...
date = 'csv_21cls_3_new'#此处为document中的日期文件夹名称 #csv_sum_adapted1_rearranged #csv_sum_22cls #csv_sum_adapted1_rearranged
...
parser.add_argument('--epochs', type=int, default=20, help='Number of epochs to train.')#初始为5
...
parser.add_argument('--batchnorm_dim', type=int, default=30, help='Batchnormalization dimension for GraphSN layer.')#修改batchnorm为64
...
parser.add_argument('--seed', type=int, default=3407, help='Random seed.')#3407
parser.add_argument('--torchseed', type=int, default=42, help='Random seed for reproducibility')#主要是torch的随机种子，包括掩码边的选择策略
#parser.add_argument('--rand_seed', type=int, default=1, help='Random seed.')#test
...
def test(test_loader, stage=0):
    ...
    with torch.no_grad():
        ...
        '''
        #只对旧类别的分类结果绘制混淆矩阵
        test_loss_old, correct_old, n_samples_old = 0, 0, 0
        preds_list = []
        data_list = []
        for batch_idx, data in enumerate(old_test_loader):
            ...
            output = model(data, res = 0)
            loss = loss_fn(output, data[4], reduction='sum')
            test_loss_old += loss.item()
            n_samples_old += len(output)
            pred = output.detach().cpu().max(1, keepdim=True)[1]
            ##################################
            data_list += data[4].tolist()#这里的形状为torch.Tensor([64])
            preds_list += pred.tolist()
            ...
            input("输入任意键继续")
            correct_old += pred.eq(data[4].detach().cpu().view_as(pred)).sum().item()

        labels_old = torch.Tensor(data_list)
        preds_old = torch.Tensor(preds_list)

        test_loss_old /= n_samples_old
        acc_old = 100. * correct_old / n_samples_old
        ...
        #绘图并保存
        if epoch >= args.epochs - 10:#只绘制最后10个epoch的混淆矩阵
            ...
            plt.tight_layout()
            plt.savefig(f'inc_test_matrix_22/{stage}/oldcls_only/fig{epoch}.png')#保存到对应路径
            plt.close()
        ...
        #只对新类别的分类结果绘制混淆矩阵
        test_loss_new, correct_new, n_samples_new = 0, 0, 0
        preds_list = []
        data_list = []
        for batch_idx, data in enumerate(new_test_loader):
            for i in range(len(data)):
                data[i] = data[i].to(args.device)
            output = model(data, res = 0)
            loss = loss_fn(output, data[4], reduction='sum')
            test_loss_new += loss.item()
            n_samples_new += len(output)
            pred = output.detach().cpu().max(1, keepdim=True)[1]
            ##################################
            data_list += data[4].tolist()#这里的形状为torch.Tensor([64])
            preds_list += pred.tolist()
            #################################
            #验证
            correct_new += pred.eq(data[4].detach().cpu().view_as(pred)).sum().item()

        labels_new = torch.Tensor(data_list)
        preds_new = torch.Tensor(preds_list)

        test_loss_new /= n_samples_new
        acc_new = 100. * correct_new / n_samples_new
        ...
        if epoch >= args.epochs - 10:#只绘制最后10个epoch的混淆矩阵
            ...
            plt.title(f'Confusion Matrix Task {stage}')
            plt.tight_layout()
            plt.savefig(f'inc_test_matrix_22/{stage}/newcls_only/fig{epoch}.png')#保存到对应路径
            plt.close()

        #清理
        del preds_list
        del data_list
        return acc, acc_old, acc_new
        '''
##################################################################
def plot_confusion_matrix(conf_matrix, num_classes, epoch, stage, path='new'):#在test中调用1次
    ...
    plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig{epoch}.png', format='png')
    '''
    if epoch == args.epochs - 10:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig0.png', format='png')
    if epoch == args.epochs - 9:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig1.png', format='png')
    if epoch == args.epochs - 8:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig2.png', format='png')
    if epoch == args.epochs - 7:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig3.png', format='png')
    if epoch == args.epochs - 6:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig4.png', format='png')
    if epoch == args.epochs - 5:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig5.png', format='png')
    if epoch == args.epochs - 4:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig6.png', format='png')
    if epoch == args.epochs - 3:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig7.png', format='png')
    if epoch == args.epochs - 2:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig8.png', format='png')
    if epoch == args.epochs - 1:
        plt.savefig(f'./inc_test_matrix_22/{stage}/{cur_path}/fig9.png', format='png')
    '''
    #plt.show()
    plt.close()
...
randseed_num = 1#随机种子数量
#randseed_list = [1, 2, 3, 4, 5]#不同的随机种子列表

for idx in range(randseed_num):
    #随机生成不同的随机种子
    ...
    datareader = DataReader(#data_dir='./data/%s/' % args.dataset.upper(),
                            ...
                            folds=args.n_folds,
                            use_cont_node_attr=False,
                            new_state=rand_seed)#单独用于随机回放的随机种子
    ...
    for fold_id in range(args.n_folds):
        ...
        model = GNN(input_dim=loaders0[0].dataset.features_dim,
                    ...
                    min_dim=loaders0[0].dataset.features_min).to(args.device)
        ...
        max_acc = 0.0
        t_start = time.time()
        for epoch in range(args.epochs):#init_cls为初始类别数，以21分类为例就是12类，在第一阶段设置较低的epoch数量加快实验#现已更正为全部采用5个epoch
            train(loaders0[0])
            #for idx in range(3):
            acc = test(loaders0[1],0)#测试轮数为3，观察测试后的内存占用情况
            accuracy_arr0[fold_id][epoch] = acc
            max_acc = max(max_acc, acc)
            if max_acc == acc:
                acc_max_epoch0 = epoch
            #新增了对新旧类别的最大acc记录
            #max_acc_old = max(max_acc_old, acc_old)
            #max_acc_new = max(max_acc_new, acc_new)
        ...
        for epoch in range(args.epochs):
            train(loaders1[0])
            acc = test(loaders1[1],1)
            accuracy_arr1[fold_id][epoch] = acc
            max_acc = max(max_acc, acc)
            if max_acc == acc:
                acc_max_epoch1 = epoch
            #新增了对新旧类别的最大acc记录
            #max_acc_old = max(max_acc_old, acc_old)
            #max_acc_new = max(max_acc_new, acc_new)
        ...
        #print(acc_folds1)
        time2 = time.time()
        print(f"训练完成，耗时{time2-time0:.2f}秒")#训练耗时
        '''
        '''
        shared_params(model, mlps_begin, mlps_end, method='mlps_begin_end')
        acc_folds2 = []
        ...
        for epoch in range(args.epochs):
            train(loaders2[0])
            acc = test(loaders2[1],2)
            accuracy_arr2[fold_id][epoch] = acc
            max_acc = max(max_acc, acc)
            if max_acc == acc:
                acc_max_epoch2 = epoch
            #新增了对新旧类别的最大acc记录
            #max_acc_old = max(max_acc_old, acc_old)
            #max_acc_new = max(max_acc_new, acc_new)
        ...
        for epoch in range(args.epochs):
            train(loaders3[0])
            acc = test(loaders3[1],3)
            accuracy_arr3[fold_id][epoch] = acc
            max_acc = max(max_acc, acc)
            if max_acc == acc:
                acc_max_epoch3 = epoch
            #新增了对新旧类别的最大acc记录
            #max_acc_old = max(max_acc_old, acc_old)
            #max_acc_new = max(max_acc_new, acc_new)
        ...
        #print(acc_folds3)
        #print(f"四个阶段的最高acc分别为{acc_folds0}、{acc_folds1}、{acc_folds2}、{acc_folds3}")#
        #print(f"四个阶段的整体最高acc出现的epoch位置为{acc_max_epoch0}、{acc_max_epoch1}、{acc_max_epoch2}、{acc_max_epoch3}")#
        #写入txt
        time_end = time.time()
        with open(f'./inc_test_matrix_22/result.txt', 'a') as f:
            f.write(f"当前的随机种子为{rand_seed},epoch数量为{args.epochs},冻结的层数为{(mlps_end-mlps_begin)}\n")
            f.write(f"四个阶段的最高acc分别为{acc_folds0}、{acc_folds1}、{acc_folds2}、{acc_folds3}\n")
            f.write(f"四个阶段的整体最高acc出现的epoch位置为{acc_max_epoch0}、{acc_max_epoch1}、{acc_max_epoch2}、{acc_max_epoch3}\n")
            f.write(f"结果已保存，当前随机种子{rand_seed}的训练总耗时为{time_end-time0:.2f}秒\n")#
