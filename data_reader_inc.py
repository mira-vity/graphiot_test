...
class DataReader():
    ...
    def __init__(self,
        ...
        try: 
            with open(pkl_path, 'rb') as f:
                data['adj_list'] = pickle.load(f)
                print(f"adj_list.pkl文件已加载，权重矩阵复合完毕")
            
        except FileNotFoundError:    
            #增加对['adj_list']附加边权重的操作
            print(f"adj_list.pkl文件未找到，尝试初次生成邻接矩阵")
            #加载索引（已迁移至main）
        ...
        #选增量1组
        #train_ids_before1 = self.random_select_examplers(train_ids0[0], 0, n_classes-9, labels, 100, 'before_numbers')#从初始化的集合里选
        #train_ids1 = self.random_select_examplers(train_ids[0], n_classes-9, n_classes-6, labels)#新类别和旧类别基本保持1:1
        
        train_ids_before1 = self.random_select_examplers(train_ids0[0], 0, n_classes-9, labels, 100, 'before_numbers')
        train_ids1 = self.random_select_examplers(train_ids[0], n_classes-9, n_classes-6, labels, ((n_classes-9) * 100)//3, 'before_numbers')
        train_ids_all1_ = train_ids_before1 + train_ids1#count数量的旧类+旧类数量*100//3的新类*3
        ...
        #选增量2组
        #train_ids_before2 = self.random_select_examplers(train_ids_all1[0], 0, n_classes-6, labels, 100, 'before_numbers')
        #train_ids2 = self.random_select_examplers(train_ids[0], n_classes-6, n_classes-3, labels)

        train_ids_before2 = self.random_select_examplers(train_ids_all1[0], 0, n_classes-6, labels, 100, 'before_numbers')
        train_ids2 = self.random_select_examplers(train_ids[0], n_classes-6, n_classes-3, labels, ((n_classes-6) * 100)//3, 'before_numbers')
        ...
        #选增量3组
        #train_ids_before3 = self.random_select_examplers(train_ids_all2[0], 0, n_classes-3, labels, 100, 'before_numbers')
        #train_ids3 = self.random_select_examplers(train_ids[0], n_classes-3, n_classes, labels)
        
        train_ids_before3 = self.random_select_examplers(train_ids_all2[0], 0, n_classes-3, labels, 100, 'before_numbers')
        train_ids3 = self.random_select_examplers(train_ids[0], n_classes-3, n_classes, labels, ((n_classes-3) * 100)//3, 'before_numbers')
    ...
    def stratified_split_data(self, labels, seed, folds):#分层分割数据
...
def shared_params(model, begin_layers=0, end_layers=0, method='None'):#用于控制参数传递，外部传参begin=1，end=3
