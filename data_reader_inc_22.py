...
class DataReader():
    ...
    def __init__(self,
        ...
        saved_pkl_path = os.path.join(self.data_dir, 'adj_list_saved_42.pkl')
        try: 
            ...
            for itr in np.arange(dataset_length):
                ...
                #weight2 = weight / weight.sum(0, keepdim=True)#列归一化

                weight = weight + torch.FloatTensor(A_array)
                #weight2 = weight2 + torch.FloatTensor(A_array)

                coeff = weight.sum(1, keepdim=True)
                coeff = torch.diag((coeff.T)[0])
                #coeff2 = weight2.sum(1, keepdim=True)#对角线还是采取原本的消息聚合维度
                #coeff2 = torch.diag((coeff2.T)[0])

                weight = weight + coeff
                #weight2 = weight2 + coeff2
                ...
                #weight2 = np.nan_to_num(weight2, nan=0)
                #weight2 = np.nan_to_num(weight2)

                #input(f"形状为{weight.shape}的复合权重矩阵已生成，按任意键继续...")
                #将两个复合权重矩阵在dim1上拼接
                #weight_mix = np.concatenate((weight, weight2), axis=1)
                #形状再次检验
                #input(f"形状为{weight_mix.shape}的拼接后权重矩阵已生成，按任意键继续...")
                data['adj_list'][itr] = weight#weight_mix
        ...
        self.rnd = random.Random(self.new_state) if self.new_state is not None else random#只生成一次随机数序列，之后持续使用
        ...
        train_ids_all1 = []
        test_ids_all1 = []
        test_ids_new1 = []#对当前阶段来说的新类别
        '''
        '''
        #2次增量
        train_ids_all2 = []
        test_ids_all2 = []
        test_ids_new2 = []#对当前阶段来说的新类别
        #3次增量
        train_ids_all3 = []
        test_ids_all3 = []
        test_ids_new3 = []#对当前阶段来说的新类别

        train_ids0_ = self.random_select_examplers(train_ids[0], 0, n_classes-9, labels)#-3是因为后面1组的数量为3*1，初始化时不做
        ...
        train_ids_before1 = self.random_select_examplers(train_ids0[0], 0, n_classes-9, labels, 1000//12, 'before_numbers' )#200
        ...
        #添加新类别
        test_ids_new1_ = self.random_select_examplers(test_ids[0], n_classes-9, n_classes-6, labels)#新类别截取
        test_ids_new1.append(np.array(test_ids_new1_))
        ...
        train_ids_before2 = self.random_select_examplers(train_ids_all1[0], 0, n_classes-6, labels, 1000//15, 'before_numbers')#train_ids[0] #200
        ...
        #添加新类别
        test_ids_new2_ = self.random_select_examplers(test_ids[0], n_classes-6, n_classes-3, labels)
        test_ids_new2.append(np.array(test_ids_new2_))
        ...
        train_ids_before3 = self.random_select_examplers(train_ids_all2[0], 0, n_classes-3, labels, 1000//18, 'before_numbers')#train_ids[0] #200
        ...
        #添加新类别
        test_ids_new3_ = self.random_select_examplers(test_ids[0], n_classes-3, n_classes, labels)
        test_ids_new3.append(np.array(test_ids_new3_))
        ...
        data['features_min'] = features_min
    ...
    def stratified_split_data(self, labels, seed, folds):#分层分割数据，还需要修改
        ...
        sss = StratifiedShuffleSplit(n_splits=1, test_size=0.3, random_state=seed)#此处不管传入randomseed还是interger，本质都一样
        ...
        for labels in range(num_classes_begin, num_classes_end):
            if len(graphs_part[labels]) > count:
                ...
                if method == 'before_numbers':
                    ...
                    #input(f"随机数种子new_state：{self.new_state}")
...
def shared_params(model, begin_layers, end_layers, method='None'):#
