...
date = 'csv_21cls_3'#此处为document中的日期文件夹名称，原本为#csv_sum_2cls_800_22cls_rearranged #csv_sum_adapted3_2cls_800 #csv_sum_22cls
...
def parse_option():
    ...
    parser.add_argument('--distill_power', type=float, default=10.0, help='Weight for distillation loss')#原本是1.0
...
def train(train_loader, model, model2, criterion, optimizer, epoch, args, positive_loader, average_feat_matrix):#这里的criterion仅用于计算非平衡的supcon损失
    ...
    for batch_idx, positive_data in enumerate(positive_loader):#消减
        ...
        if args.target_task > 0:
            ...
            features1_prev_task = torch.cat([feat,feat],dim = 0)#将当前任务和上一任务的特征拼接在一起，沿着batch维度拼接 #_positive
        ...
        #feat2 = feat
        features = torch.cat([feat.unsqueeze(1), feat.unsqueeze(1)], dim=1)#将原始特征和噪声特征拼接成一个batch，形成多视角输入 #_positive
        ...
        if args.target_task > 0:
            ...
            with torch.no_grad():
                ...
                features2_prev_task = torch.cat([feat2,feat2],dim = 0)#将当前任务和上一任务的特征拼接在一起，沿着batch维度拼接 #_positive
    ...
    return loss, model2, cur_fore_feat, average_feat_matrix
...
def main():
    ...
    for target_task in range(0, args.end_task+1):#0，1
        ...
        for epoch in range(1, epochs[target_task] + 1):
            ...
            time1 = time.time()
            if adj_mask == 1:
                loss, model2, _1st_batch_feat, average_feat_matrix_tmp = train([], model, model2, criterion, optimizer, epoch, args, loaders_positive[0], average_feat_matrix)#origin
            else:
                loss, model2, _1st_batch_feat, average_feat_matrix_tmp = train(loaders[0], model, model2, criterion, optimizer, epoch, args, loaders[0], average_feat_matrix)#即没有掩码的正样本，但传入两个相同引用导致之后迭代时的迭代状态独立（错误用法）
            '''
