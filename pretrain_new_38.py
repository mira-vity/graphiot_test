...
def dataloading(datareader, mode = 0):#这里需要对原始的datareader加载的pkl和正样本加载的pkl作出区分，mode0为原始的连接矩阵，mode1是新的掩码矩阵
    if mode == 1:#掩码矩阵，保存到af_adjlist中
        ...
        saved_path = os.path.join('./dataprocess/document/'+str(date)+'/usable_te', 'te_af_adj_list_42_all_0.5random.pkl')#'./dataprocess/document/'+str(date)+'/usable_te/'
        #尝试读取带权邻接矩阵
        try: 
...
def parse_option():
    ...
    # 训练参数
    parser.add_argument('--epochs', type=int, default=10, help='Number of training epochs')#原本是10，设置为1时是因为要测networkx的构图效率
    ...
    #SGD优化器参数（set_opt）
    parser.add_argument('--learning_rate', type=float, default=0.001)#由于优化器的不同，adam实际采用的学习率是这个#原本为0.05
    ...
    parser.add_argument('--current_temp', type=float, default=0.2)#原先是#1.0
    parser.add_argument('--past_temp', type=float, default=0.01)#原先是#1.0
    parser.add_argument('--distill_power', type=float, default=10.0, help='Weight for distillation loss')#原本是1.0 #0.5 #1.0
    ...
    parser.add_argument('--batchnorm_dim', type=int, default=30, help='batchnorm_dim')#理应修改为和hidden_dim相同
    ...
    parser.add_argument('--lr', type=float, default=0.0001, help='Learning rate for optimizer')#原本为0.0025#在此之前为0.001
    parser.add_argument('--rand_seed', type=int, default=42, help='Random seed for replaced_sample generation')#单独用于样本回放的随机种子，原42（可能不需要随预训练种子进行随时更改）
...
def train(train_loader, model, model2, criterion, optimizer, epoch, args, positive_loader):#这里的criterion仅用于计算非平衡的supcon损失
    ...
    for batch_idx, (data, positive_data) in enumerate(zip(train_loader, positive_loader)):
        ...
        #IRD_past
        if args.target_task > 0:
            model2.eval()#关闭batchnorm
            with torch.no_grad():
                feat2 = model2(data)
                feat2_positive = model2(positive_data)#重要修正，测试是否需要model2.eval
    ...
    return loss, model2, cur_fore_feat
...
def main():
    ...
    dataset = [['train0'], ['train1'],['train2'],['train3']]#消减
    ...
    for target_task in range(0, args.end_task+1):#0，1
        ...
        for epoch in range(1, epochs[target_task] + 1):
            ...
            time1 = time.time()
            if adj_mask == 1:
                loss, model2, _1st_batch_feat = train(loaders[0], model, model2, criterion, optimizer, epoch, args, loaders_positive[0])#origin
            else:
                loss, model2, _1st_batch_feat = train(loaders[0], model, model2, criterion, optimizer, epoch, args, loaders[0])#即没有掩码的正样本，但传入两个相同引用导致之后迭代时的迭代状态独立（错误用法）
            '''
        ...
        #保存模型
        save_path = os.path.join('./checkpoint/cls21', 'model_task_{}.pth'.format(target_task))#虽然写成cls38，实际上是用于UNSW旧的数据集22cls的2阶段分类
