...
class DataReader():
    ...
    def __init__(self,
        ...
        #尝试读取带权邻接矩阵
        pkl_path = os.path.join(self.data_dir, 'adj_list_gat.pkl')
        ...
        try: 
            with open(pkl_path, 'rb') as f:
                ...
                print(f"adj_list_gat.pkl文件已加载，权重矩阵复合完毕")
            
        except FileNotFoundError:    
            #增加对['adj_list']附加边权重的操作
            print(f"adj_list_gat.pkl文件未找到，尝试初次生成邻接矩阵")#计算时间
            #加载索引（已迁移至main）
        ...
        '''
        train_ids0_ = self.random_select_examplers(train_ids[0], 0, n_classes-3, labels)#-3是因为后面1组的数量为3*1，初始化时不做
        #input(f"初始化类别的长度：{len(train_ids0_)}")
        test_ids0_ = self.random_select_examplers(test_ids[0], 0, n_classes-3, labels)
        ...
        train_ids_before1 = self.random_select_examplers(train_ids0[0], 0, n_classes-3, labels, 500, 'before_numbers')
        train_ids1 = self.random_select_examplers(train_ids[0], n_classes-3, n_classes-0, labels, ((n_classes-3) * 500)//3, 'before_numbers')
        ...
        test_ids_all1_ = self.random_select_examplers(test_ids[0], 0, n_classes-0, labels)#测试集更新
        test_ids_all1.append(np.array(test_ids_all1_))
        #添加新类别
        test_ids_new1_ = self.random_select_examplers(test_ids[0], n_classes-3, n_classes-0, labels)#新类别截取
        ...
        train_ids_before2 = self.random_select_examplers(train_ids_all1[0], 0, n_classes-6, labels, 500, 'before_numbers')
        train_ids2 = self.random_select_examplers(train_ids[0], n_classes-6, n_classes-3, labels, ((n_classes-6) * 500)//3, 'before_numbers')
        ...
        train_ids_before3 = self.random_select_examplers(train_ids_all2[0], 0, n_classes-3, labels, 500, 'before_numbers')
        train_ids3 = self.random_select_examplers(train_ids[0], n_classes-3, n_classes, labels, ((n_classes-3) * 500)//3, 'before_numbers')
