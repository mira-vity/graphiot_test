...
date = 'csv_sum_22cls'#此处为document中的日期文件夹名称，原本为 csv_sum_adapted3_2cls_800
...
def parse_option():
    ...
    # 训练参数
    parser.add_argument('--epochs', type=int, default=100, help='Number of training epochs')#原本是5，设置为1时是因为要测networkx的构图效率
    parser.add_argument('--target_task', type=int, default=0)
    parser.add_argument('--n_cls', type=int, default=22)#原本为21
    parser.add_argument('--init_cls', type=int, default=19)#原本为12
    parser.add_argument('--cls_per_task', type=int, default=3)
    parser.add_argument('--batch_size', type=int, default=30)#原本为64，影响特征的第0维度
    parser.add_argument('--output_dim', type=int, default=22)#原本为21
...
def train(train_loader, model, model2, criterion, optimizer, epoch, args):#这里的criterion仅用于计算非平衡的supcon损失
    ...
    for batch_idx,data in enumerate(train_loader):
        ...
        #print(f"feat的形状{feat.shape}")#打印feat的形状(30,22)
        #
...
def main():
    ...
    for target_task in range(0, args.end_task+1):#0，1
        ...
        #保存模型
        save_path = os.path.join('./checkpoint/cls22/new', 'model_task_{}.pth'.format(target_task))
